...
def _apply_centering_priority(ctrl: AirSimController, decision: dict, altitude: float, min_altitude: float) -> str:
    if decision.get("plane_visible") and not decision.get("centered") and decision.get("target_offset") not in {"unknown", "center"}:
        return _center_target(ctrl, decision["target_offset"])
    if decision.get("destroyed_area_visible") and not decision.get("centered") and decision.get("target_offset") not in {"unknown", "center"}:
        return _center_target(ctrl, decision["target_offset"])
    return _apply_action(ctrl, decision, altitude, min_altitude)


def _log_decision(stage: str, step: int, decision: dict, altitude: float | None = None) -> None:
    if altitude is None:
        logger.info(
            "[%s %02d] action=%s visible=%s offset=%s centered=%s descend=%s conf=%s reason=%s",
            stage,
            step,
            decision["action"],
            decision.get("destroyed_area_visible", decision.get("plane_visible")),
...
def _center_plane_before_next_stage(
    ...
    for step in range(1, max_steps + 1):
        ...
        plane_image = image
        _save_image(image, input_dir, f"{image_prefix}_{step:02d}_input.jpg")
        altitude = ctrl.get_altitude()
        decision = agent.decide_plane(image=image, altitude=altitude, last_action=last_action)
        _log_decision(stage_name, step, decision, altitude=altitude)

        if decision["plane_visible"] and decision["centered"]:
            logger.info("飞机已在 %.1f 米附近居中", altitude)
        ...
        last_action = _apply_centering_priority(ctrl, filtered_decision, altitude, min_altitude=altitude_target)
...
def run_task(
    ...
    try:
        ...
        for step in range(1, max_area_steps + 1):
            ...
            _log_decision("area", step, decision)
            ...
            if decision["destroyed_area_visible"] and decision["centered"]:
                ...
                area_image_path = _save_image(image, str(result_dir), "destroyed_area_topdown.jpg")
                logger.info("已锁定炸毁区域并保存俯视图: %s", area_image_path)
                logger.info("保持当前150米高度，进入飞机搜索阶段")
                last_action = "hover"
                break

            last_action = _apply_centering_priority(ctrl, decision, altitude, min_altitude=PLANE_LOCK_ALTITUDE)
            _sleep_between_steps()
        else:
        ...
        for step in range(1, max_plane_steps + 1):
            ...
            _save_image(image, str(input_dir), f"plane_step_{step:02d}_input.jpg")

            altitude = ctrl.get_altitude()
            decision = agent.decide_plane(image=image, altitude=altitude, last_action=last_action)
            _log_decision("plane", step, decision, altitude=altitude)
        ...
        logger.info("阶段3/4：下降到100米并执行居中")
        ...
        plane_image, last_action, lock_centered = _center_plane_before_next_stage(
            ...
            image_prefix="plane_lock_step",
            stage_name="plane-lock",
            last_action=last_action,
        )
        if not lock_centered:
            raise RuntimeError("飞机已发现，但在100米锁定阶段未能完成居中。")

        current_altitude = ctrl.get_altitude()
        logger.info("飞机已居中，开始下降到50米进行评估")
        ...
        for step in range(1, MAX_EVAL_CENTER_STEPS + 1):
            ...
            plane_image = image
            _save_image(image, str(input_dir), f"plane_eval_step_{step:02d}_input.jpg")
            altitude = ctrl.get_altitude()
            decision = agent.decide_plane(image=image, altitude=altitude, last_action=last_action)
            if decision["action"] == "move_down":
                decision["action"] = "hover"
                decision["parameters"] = {"duration": 1.0}
                decision["should_descend"] = False
            _log_decision("plane-eval", step, decision, altitude=altitude)

            if decision["plane_visible"] and decision["centered"]:
                eval_centered = True
                logger.info("飞机已在 %.1f 米评估高度附近重新居中", altitude)
                break

            last_action = _apply_centering_priority(ctrl, decision, altitude, min_altitude=PLANE_EVAL_ALTITUDE)
            _sleep_between_steps()

        if not eval_centered:
            logger.warning("在50米评估高度未再次确认目标居中，将使用最后一帧执行评估")

        if plane_image is None:
            raise RuntimeError("缺少飞机图像，无法执行毁伤评估。")

        logger.info("阶段4/4：50米高度执行飞机毁伤评估")
        plane_eval_image_path = _save_image(plane_image, str(input_dir), "plane_eval_input.jpg")
        ...
        final_plane_image_path = _save_image(plane_image, str(result_dir), "plane_topdown.jpg")
        logger.info("已保存最终评估视角俯视图: %s", final_plane_image_path)
        logger.info("评估输入图像: %s", plane_eval_image_path)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="任务输出根目录，脚本会在该目录下使用 input/ 和 output/",
    )
